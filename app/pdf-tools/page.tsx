...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  const cards = [
    ...
    { ico: "📝", title: "PDF → DOCX", desc: "Convert PDF text into an editable Word document", col: "#3b82f6", tip: "Extracts text from each page and builds a real .docx file", body: <><FZone accept=".pdf" label="Drop a PDF here" file={g("pdfDocx").file} onFile={(f: File) => s("pdfDocx", { file: f })} tip="The PDF to convert to Word format" />{g("pdfDocx").loading && (g("pdfDocx").prog || 0) > 0 && <div className="h-1.5 bg-paper3 rounded-full overflow-hidden border border-paper3"><div className="h-full bg-amber transition-all duration-300 rounded-full" style={{ width: (g("pdfDocx").prog || 0) + "%" }} /></div>}<CStat msg={g("pdfDocx").status} type={g("pdfDocx").statusType} /><HBtn onClick={convFns.pdfDocx} disabled={!g("pdfDocx").file} loading={g("pdfDocx").loading} label="Convert to DOCX" tip="Creates an editable .docx file from PDF text" /></> },
    { ico: "🔗", title: "PDF → Link", desc: "Upload & get a shareable viewer link", col: "#10b981", tip: "Like Tiiny.host — upload your PDF and share a link", body: <div className="flex flex-col items-center gap-4 py-4 relative"><div className="absolute -top-4 -right-4 bg-teal text-white text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-full shadow border border-teal text-center z-10">Beta</div><div className="text-sm font-medium text-ink4 text-center leading-relaxed">Upload your PDF and get an instant shareable link that anyone can open to view and download.</div><Link href="/pdf-link" className="py-3 px-6 bg-amber hover:bg-amber2 text-white text-[15px] font-bold rounded-full shadow-sm hover:shadow hover:-translate-y-0.5 transition-all duration-200 no-underline cursor-pointer flex items-center justify-center gap-2 w-full"><Emoji symbol="🔗" size={18} /> Go to PDF Link</Link></div> },
  ];
