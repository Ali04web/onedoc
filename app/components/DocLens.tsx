...
export default function DocLens() {
  ...
  const innerTabs = [
    { id: "text", l: "📄 Text", t: "Read the extracted document text" },
    { id: "stats", l: "📊 Stats", t: "Word counts and frequency analysis" },
    { id: "search", l: "🔍 Search", t: "Find any word in the document" },
    { id: "export", l: "💾 Export", t: "Download in different file formats" },
  ];
    ...
    <div className="flex flex-1 overflow-hidden min-h-0 relative">
      ...
      <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:relative z-[80] w-[280px] md:w-[264px] border-r-2 border-dashed border-[rgba(100,70,40,.18)] flex flex-col overflow-hidden bg-paper md:bg-[rgba(237,229,208,.45)] flex-shrink-0 h-full transition-transform duration-200`}>
        ...
        <div className="md:hidden flex items-center justify-between px-[14px] py-[10px] border-b border-[rgba(100,70,40,.15)]">
          <span className="font-caveat text-[18px] font-bold text-ink2">📂 Documents</span>
          <button onClick={() => setSidebarOpen(false)} className="bg-transparent border-none text-ink4 cursor-pointer text-[18px]">✕</button>
        </div>
      ...
      <main className="flex flex-col flex-1 overflow-hidden bg-transparent min-w-0">
        ...
        {!active ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-[14px] p-8 md:p-[48px] text-center relative overflow-hidden">
            <div className="text-[48px] md:text-[60px] inline-block font-sans animate-wobble-in -rotate-[8deg]">📄</div>
            ...
            <button
              ...
              className="md:hidden mt-4 py-[10px] px-[22px] bg-amber hover:bg-amber2 text-white font-caveat text-[16px] font-bold rounded-[3px_10px_4px_9px] border-2 border-amber2 shadow-[2px_2px_0_rgba(30,15,5,.12)] cursor-pointer transition-all duration-150"
            >
              📂 Open Sidebar
            </button>
          </div>
